CREATE SCHEMA IF NOT EXISTS `local-project.cd_tenant_SMB_1042`
OPTIONS(location="asia-northeast1");

CREATE TABLE IF NOT EXISTS `local-project.cd_tenant_SMB_1042.onboarding_answer_events` (
...
CREATE TABLE IF NOT EXISTS `local-project.cd_tenant_SMB_1042.followup_answer_events` (
...
CREATE TABLE IF NOT EXISTS `local-project.cd_tenant_SMB_1042.kpi_candidates` (
...
CREATE TABLE IF NOT EXISTS `local-project.cd_tenant_SMB_1042.focus_metric_candidates` (
...
CREATE TABLE IF NOT EXISTS `local-project.cd_tenant_SMB_1042.current_kpi_definitions` (
...
CLUSTER BY company_id, kpi_domain, status;

CREATE TABLE IF NOT EXISTS `local-project.cd_tenant_SMB_1042.current_focus_metric_definitions` (
...
CLUSTER BY company_id, metric_category, status;

CREATE OR REPLACE PROPERTY GRAPH `local-project.cd_tenant_SMB_1042.KnowledgeGraph`
NODE TABLES (
  `local-project.cd_tenant_SMB_1042.knowledge_nodes`
    KEY (node_id)
    LABEL KnowledgeNode
    PROPERTIES (company_id, node_type, label, description, confidence, status, properties)
)
EDGE TABLES (
  `local-project.cd_tenant_SMB_1042.knowledge_edges`
