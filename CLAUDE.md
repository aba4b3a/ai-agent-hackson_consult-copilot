...
Claude Code follows the same base operating rules as `AGENTS.md`. This file adds Claude-specific entry points.

## Shared Rules

Read `AGENTS.md` first. It is the source of truth for repository-wide behavior, safety, MCP, schema validation, cost, and release-gate rules. Nothing in this file overrides it.

## Skill Usage

- Skills live under `.claude/skills/`, generated from `.ai-common/skills/`.
- `SKILLS.md` is the index. Procedures live in `skills/*/SKILL.md`.
- One skill = one task workflow.
- Skills with side effects (deploy, release, Terraform apply, IAM, secret access, destructive ops) declare `disable-model-invocation: true` in their YAML frontmatter — Claude must never auto-trigger them. They run only when a human invokes them.
- Before any side-effecting step, restate intent, affected resources, required approval, and rollback path.

## MCP Usage

- `.mcp.json` is the active permission boundary.
- Default deny; read-only first; writes require human approval per invocation.
- The base `.mcp.json` only enables read-only / local servers (`filesystem`, `playwright`).
- Other servers documented under `mcp/servers/` exist as policy reference; enabling them is a project-overlay decision with explicit approval.

## Workflow

1. Read the PR diff or the user request.
2. Plan the smallest change that resolves it.
3. Use a skill if one matches; otherwise fall back to base behavior in `AGENTS.md`.
4. Report findings against schemas where applicable.
5. Do not execute production, IAM, secret, or destructive operations.

## Project Overlays

Apply `.ai/overlays/CLAUDE.project.md` after this file. Overlays may add stricter rules or project-specific context; they may not relax base rules.
